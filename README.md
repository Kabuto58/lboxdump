**For a stable user experience, only download the latest release files from the release page.**
...
- As per VacciBucks, medic walks to upgrade station and begins process. Take note the threshold money count is fixed in script to the amount required by Sniper to purchase all essential MvM upgrades. Threshold may not be changed in the in-game console, unlike the original script. There is a choice to change to a money-only mode just like Vaccibucks, triggered by pressing the key P. All other features described below assumes you choose Sniper mode.
...
- Mission is auto-detected by script when joining server, and an integer value matching the map name is chosen from script as a timer duration. This timer duration has been tested to be the ideal duration to abandon a game before the end of the last wave, with a margin of error for safety. The timer can be manually toggled off by pressing the O key, in switch the script will change to a "naturally end" mode without timer.
- On the start of the last wave, timer starts counting down.
- Client abandons the game when timer is up.
- If script is in "naturall end" mode, the client will still auto-disconnect without having the need to press the disconnect button, when the natural end of game state is detected.
